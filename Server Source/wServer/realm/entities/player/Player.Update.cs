...
namespace wServer.realm.entities
{
    public partial class Player
    {
        ...
        void SendUpdate(RealmTime time)
        {
            ...
            var list = new List<UpdateMessage.TileData>(APPOX_AREA_OF_SIGHT);
            int sent = 0;
            foreach (var i in Sight.GetSightCircle(RADIUS))
            {
                int x = i.X + _x;
                ...
                list.Add(new UpdateMessage.TileData()
                {
                    X = (short)x,
                    Y = (short)y,
            ...
            {
                UpdateMessage packet = new UpdateMessage();
                packet.tiles_ = list.ToArray();
                packet.newObjs_ = sendEntities.Select(_ => _.ToDefinition()).Concat(newStatics).ToArray();
                packet.drops_ = dropEntities.Concat(removedIds).ToArray();
        ...
        void SendNewTick(RealmTime time)
        {
            ...
            NewTickMessage p = new NewTickMessage();
            tickId++;
            p.tickId_ = tickId;
            p.tickTime_ = time.thisTickTimes;
            p.statuses_ = sendEntities.Select(_ => _.ExportStats()).ToArray();
            client.SendPacket(p);

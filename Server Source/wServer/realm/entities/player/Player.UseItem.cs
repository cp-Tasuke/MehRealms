...
namespace wServer.realm.entities
{
    partial class Player
    {
        ...
        static void ActivateHealHp(Player player, int amount, List<Packet> pkts)
        {
            ...
            if (newHp != player.HP)
            {
                pkts.Add(new ShowEffectMessage()
                {
                    effectType_ = EffectType.Potion,
                    targetObjectId_ = player.Id,
                    color_ = new ARGB(0xffffffff)
                });
                pkts.Add(new NotificationPacket()
                {
                    color_ = new ARGB(0xff00ff00),
                    ObjectId = player.Id,
        ...
        void PoisonEnemy(Enemy enemy, ActivateEffect eff)
        {
            ...
            tmr = new WorldTimer(100, (w, t) =>
            {
                ...
                w.BroadcastPacket(new ShowEffectMessage()
                {
                    effectType_ = EffectType.Dead,
                    targetObjectId_ = enemy.Id,
                    color_ = new ARGB(0xffddff00)
                }, null);
                    ...
                    case ActivateEffects.BulletNova:
                        {
                            ...
                            batch[20] = new ShowEffectMessage()
                            {
                                effectType_ = EffectType.Trail,
                                pos1_ = target,
                                targetObjectId_ = Id,
                                color_ = new ARGB(0xFFFF00AA)
                            };
                    ...
                    case ActivateEffects.StatBoostSelf:
                        {
                            ...
                            BroadcastSync(new ShowEffectMessage()
                            {
                                effectType_ = EffectType.Potion,
                                targetObjectId_ = Id,
                                color_ = new ARGB(0xffffffff)
                    ...
                    case ActivateEffects.StatBoostAura:
                        {
                            ...
                            BroadcastSync(new ShowEffectMessage()
                            {
                                effectType_ = EffectType.AreaBlast,
                                targetObjectId_ = Id,
                                color_ = new ARGB(0xffffffff),
                                pos1_ = new Position() { X = eff.Range / 2 }
                            }, p => this.Dist(p) < 25);
                        } break;
                    ...
                    case ActivateEffects.Teleport:
                        {
                            ...
                            BroadcastSync(new Packet[]
                            {
                                ...
                                new ShowEffectMessage()
                                {
                                    effectType_ = EffectType.Teleport,
                                    targetObjectId_ = Id,
                                    pos1_ = new Position()
                                    {
                                        X = X,
                    ...
                    case ActivateEffects.VampireBlast:
                        {
                            ...
                            for (int i = 0; i < 5; i++)
                            {
                                ...
                                pkts.Add(new ShowEffectMessage()
                                {
                                    effectType_ = EffectType.Flow,
                                    targetObjectId_ = b.Id,
                                    pos1_ = new Position() { X = a.X, Y = a.Y },
                                    color_ = new ARGB(0xffffffff)
                                });
                            }
                    ...
                    case ActivateEffects.StasisBlast:
                        {
                            ...
                            pkts.Add(new ShowEffectMessage()
                            {
                                effectType_ = EffectType.Concentrate,
                                targetObjectId_ = Id,
                                pos1_ = target,
                                pos2_ = new Position() { X = target.X + 3, Y = target.Y },
                                color_ = new ARGB(0xffffffff)
                            });
                    ...
                    case ActivateEffects.Lightning:
                        {
                            ...
                            for (int i = 0; i < targets.Length; i++)
                            {
                                ...
                                pkts.Add(new ShowEffectMessage()
                                {
                                    effectType_ = EffectType.Lightning,
                                    targetObjectId_ = prev.Id,
                                    color_ = new ARGB(0xffff0088),
                                    pos1_ = new Position()
                                    {
                                        X = targets[i].X,
                                        Y = targets[i].Y
                                    },
                                    pos2_ = new Position() { X = 350 }
                                });
                            }
                            BroadcastSync(pkts, p => this.Dist(p) < 25);
                        } break;
                    case ActivateEffects.PoisonGrenade:
                        {
                            ...
                            Owner.Timers.Add(new WorldTimer(1500, (world, t) =>
                            {
                                Owner.BroadcastPacket(new ShowEffectMessage()
                                {
                                    effectType_ = EffectType.AreaBlast,
                                    color_ = new ARGB(0xffddff00),
                                    targetObjectId_ = x.Id,
                                    pos1_ = new Position() { X = eff.Radius }
                                }, null);

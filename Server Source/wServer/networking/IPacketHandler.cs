...
namespace wServer.networking
{
    ...
    {
        PacketID ID { get; }
        void Handle(Client client, ClientMessage packet);
    }
    abstract class PacketHandlerBase<T> : IPacketHandler where T : ClientMessage
    {
        protected abstract void HandlePacket(Client client, T packet);

        public abstract PacketID ID { get; }

        public void Handle(Client client, ClientMessage packet)
        {
